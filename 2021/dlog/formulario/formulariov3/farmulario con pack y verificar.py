...
def insert():
    ...
    instruccion="INSERT INTO Usuarios('nombre', 'apellido', 'calle', 'numero', 'telefono', 'localidad', 'estado civil', 'nombre usuario', 'password' ) VALUES ('%s', '%s', '%s', '%d', '%d', '%s', '%d', '%s', '%s')" %(NOMBRE, APELLIDO, CALLE, NUMERO, TELEFONO, LOCALIDAD, ESTADO, USUARIO, CLAVE)
    print(instruccion)
    registro.execute(instruccion)
    base.commit()

    label_comentario=Label(ventana, text="datos agregados").grid(column=0, row=17)
    
    for fila in registros:
        datos=True
    if registro:
        label_datos=Label(ventana, text="datos agregados").grid(column=0, row=18)
    else:
        label_datos2=Label(ventana, text="hubo un error").grid(column=0, row=19)
